...
class EventLogMongoDbPersistence
  ...
  dynamic composeFilter(FilterParams filter) {
    ...
    if (search != null) {
      ...
      var searchCriteria = [];
      searchCriteria.add({
        'source': {r'$regex': searchRegex}
      });
      searchCriteria.add({
        'type': {r'$regex': searchRegex}
      });
      searchCriteria.add({
        'message': {r'$regex': searchRegex}
      });
    ...
    if (correlationId != null) criteria.add({'correlation_id': correlationId});

    var source = filter.getAsNullableString('source');
    if (source != null) criteria.add({'source': source});
